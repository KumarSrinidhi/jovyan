...
\item A detailed method rationale section that explains expected behavior of each strategy class A--E before looking at outcomes.
...
It evaluates core deep models first, then attempts advanced strategies A--E.
...
\subsection{Advanced Strategy Families A--E}
...
\section{Advanced Strategy Outcomes (A--E) with Run-Status Transparency}
...
\item interpretability payload (TabNet masks) attached only to escalated cases.
\end{enumerate}
...
The advanced strategy block (A--E) is scientifically valuable, but partial execution currently limits comparative claims. This section defines a concrete completion protocol so the next run can produce publication-grade evidence with minimal ambiguity.
...
\begin{enumerate}
\item Complete a clean rerun of advanced strategy B in a fresh runtime with memory monitoring.
...
\bibliography{ieee_nids_notebooks_grounded_refs}
